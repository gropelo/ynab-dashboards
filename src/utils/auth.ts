...
const setStorageToken = (token: string) => {
  ...
  window.location.href = process.env.REACT_APP_HOSTNAME || 'http://localhost:3000';
}
...
const requestAuthorization = () => {
  ...
  window.location.href = `https://app.youneedabudget.com/oauth/authorize?client_id=${clientId}&redirect_uri=${encodeURIComponent(process.env.REACT_APP_HOSTNAME || 'http://localhost:3000/')}&response_type=token`;
}
